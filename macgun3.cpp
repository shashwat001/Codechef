...
scanf("%d %d",&m,&n);
	...
	//{
		...
		for(i = 0;i < m;i++)
		{
			for(j = 0;j < n;j++)
			{
				dp[i][j][0] = max(max(max(dp[i-1][j][0],dp[i-1][j][1]),max(dp[i-1][j-1][0],dp[i-1][j-1][1])),max(max(dp[i][j-1][0],dp[i][j-1][1]),max(dp[i+1][j-1][0],dp[i+1][j-1][1])));
				if(a[i][j]==-1)
					dp[i][j][1] = dp[i][j][0];
				else
				{
					if(str[i-1][j-1] == 'P')
					{
						temp[0] = max(dp[i-2][j-2][0],dp[i-2][j-2][1]);
					}
					else
						temp[0] = dp[i-2][j-2][0];
					if(str[i+1][j-1] == 'P')
