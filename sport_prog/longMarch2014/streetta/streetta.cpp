...
void update2(lli b,lli e,lli node,lli i,lli j,lli a,lli d)
{
    ...
    if(b==i && e==j)
    {
        ...
        while(!M[node].l.empty())
        {
            pi p = M[node].l.back();
            M[node].l.pop_back();
            lli lasta = M[node].a1 + (vi[e] - vi[b])*M[node].d1;
            lli newlasta = p.first+(vi[e] - vi[b])*p.second;
            if(M[node].a1 < p.first && lasta < newlasta)
            {
                M[node].a1 = p.first;
                M[node].d1 = p.second;
            }
            else if(M[node].a1 >= p.first && lasta >= newlasta)
            {
            }
            else
            {
                M[2*node].l.push_back(p);
                lli newa = p.first+(vi[mid+1]-vi[b])*p.second; 
                M[2*node+1].l.push_back(mp(newa,p.second));
            }
