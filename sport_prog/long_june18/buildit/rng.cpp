...
int main()
{
    ...
    S[k] = (last + S[k-1])%mod;
    // cout<<solvesmart(SC, S, 3).x<<endl;
	num answer = solvesmart(SC, S, h);
    // cout<<answer.x<<endl;
    ...
    for(int i = 0;i < n;i++)
    {
        ...
        {
            left = p[i]+1-x + h+1;
            num temp = answer-solvesmart(SC,S,left-1);
            // cout<<temp.x<<endl;
            sump = sump + temp;
            sump = sump + solvesmart(SC,S,p[i]);
        }
        else
        {
            num temp = solvesmart(SC,S,p[i]) - solvesmart(SC,S,p[i]-x);
            // cout<<temp.x<<endl;
            sump = sump + temp;
        }
    }

    // num answer1 = solvestupid(C, A, h);
	// printf("%d\n%d\n", answer.x, answer1.x);
    printf("%d\n", (sump*num(inverse(answer.x, mod))).x);
	return 0;
}
